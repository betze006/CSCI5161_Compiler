...
void add_var ( char *type, char *name, int dim, int nest_level, int sp_offset_local, int *arr_dim) {
	if ( var_ll == NULL ) {
		var_t *var_pointer;
		...
		var_pointer->dim = dim;
		var_pointer->arr_dimms = arr_dim;
		var_pointer->sp_offset = sp_offset_local;
...
void add_var_list ( char *type, id_list_t *head, int nest_level ) {
	...
	add_var ( real_type, head->name, head->dim, nest_level, head->sp_offset, head->arr_dimms );
...
id_list_t *newIDnode(char* name, int dim){
	...
	if ( dim > 0 ) {
		temp->arr_dimms = (int *) malloc ( sizeof(int) * dim );
		int i, arr_size=4;
		for ( i=dim-1 ; i>=0 ; i-- ) {
			temp->arr_dimms[i] = arr_size;
			arr_size *= get_array_dim ( i );
		}
		temp->sp_offset = get_sp_offset();
		set_sp_offset( get_sp_offset() - arr_size );
	}
	else {
		temp->sp_offset = get_sp_offset();
...
void add_func ( char *type, char *name, param_list_t *params ) {
	...
	{
		add_var(param_list->param.type, param_list->param.name, param_list->param.dim, param_list->param.nest_level, get_sp_offset(), NULL );
		set_sp_offset( get_sp_offset() - 4 );
...
void append_decl_list_to_var( decl_list_t *decl_list )
{
	...
	{
		add_var( decl_list->var.type, decl_list->var.name, decl_list->var.dim, decl_list->var.nest_level, 1000, NULL );
